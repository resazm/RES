...
df_kokyaku["月"] = df_kokyaku["顧客登録日"].dt.month.astype(str)  # "月"の列を追加
df_kokyaku["年"] = df_kokyaku["顧客登録日"].dt.year.astype(str)  # "年"の列を追加
df_kokyaku["年月"] = df_kokyaku["顧客登録日"].dt.strftime('%Y/%m')
df_kokyaku["年月日"] = df_kokyaku["顧客登録日"].dt.strftime('%Y/%m/%d')
df_kokyaku["日付"] = df_kokyaku["顧客登録日"].dt.strftime('%d')
df_kokyaku["日付2"] = df_kokyaku["顧客登録日"].dt.strftime('%m/%d')
...
with col2:
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.subheader("")
    #tougetu_df2 = tougetu_df.groupby("オプトイン").sum(numeric_only=True).iloc[:10].reset_index() #※トップ20だけ表示
    #fig2 = px.bar(tougetu_df2.sort_values(by="オプトイン", ascending=False), x="合計金額", y="オプトイン", color="オプトイン", orientation="h", title="オプトイン別 当月注文金額合計 TOP10")
    ...
    #fig2.update_yaxes(title="顧客流入元",linecolor='black',gridcolor='lightgrey', gridwidth=1, griddash='dot',categoryorder='total ascending',tickfont_size=12)
